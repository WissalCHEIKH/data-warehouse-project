...
BEGIN
	...
	BEGIN TRY
		...
		SET @end_time = GETDATE();
		PRINT '>>  Loading Duration of bronze.crm_prod_info: ' + CAST (DATEDIFF(second, @start_time, @end_time) AS NVARCHAR) + 'seconds';
		PRINT '>> -------------------------------------------------------------------------------'
		...
		SET @end_time = GETDATE();
		PRINT '>> Loading Durationof bronze.erp_loc_a101: ' + CAST(DATEDIFF(second, @start_time, @end_time) AS NVARCHAR) + 'seconds' ;
		PRINT '>> -------------------------------------------------------------------------------'
